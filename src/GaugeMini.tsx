...
export const getColors = (theme: Required<GaugeMiniLayerConfig>): Colors => {
  ...
  gaugeColors.forEach(({ hex, name }) => d3Color(hex) ?? throwReturn(`Object "${hex}" isn"t valid color for name:${name}`));

  return {
    min: gaugeColors.find(x => x.type === "min") ?? throwReturn("color of type min must be defined"),
    max: gaugeColors.find(x => x.type === "max") ?? throwReturn("color of type max must be defined"),
    thresholds: gaugeColors.filter(({ type }) => type === "threshold").sort(({ value: a }, { value: b }) => a - b),
    secondary,
  };
};
...
const Text: FunctionComponent<TextProps> = ({ value, barValueWidth, theme }) => {
  ...
  const textAnchor = (textInside && follow)
    ? "end"
    : "start"
    ;

  const x = follow
    ? Math.max(barValueWidth + (textInside ? -1 : +1) * valuePadding, valuePadding)
    : valuePadding
    ;
...
const Bar: FunctionComponent<BarProps> = ({ value, theme, y, barWidth, getFrac, }) => {
  ...
  const valueFracFixed = Math.max(
    -oveflowFraction, Math.min(oveflowFraction + 1, getFrac(value)));
  ...
  return <g className={barCssClass}>
    <g transform={`translate(${0},${y})`}>
      ...
      <BarValue {...{ colors, theme, barValueWidth, value, valueFracFixed, barCenter }} />
...
const Axes: FunctionComponent<AxesProps> = ({ theme, barWidth, y, getFrac, }) => {
  ...
  const axesLineStyle: React.CSSProperties = { stroke: axesFontColor, strokeWidth: 2, strokeLinecap: "round" };

  const axesValuesArray = Array.isArray(axesSteps) ? axesSteps
...
export const GaugeMini: FunctionComponent<Props> = ({ value, theme, width, height }) => {
  ...
  return (
    <svg width={width} height={height} style={{ fontFamily: "Rubik, monospace", userSelect: "none" }} >
