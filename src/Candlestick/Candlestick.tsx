...
import { CANDLESTICK_THEME_DARK } from "./candlesticksStyles";
...
const Candle: React.FC<CandleProps> = ({ heightPositionFormatter, theme, value, width }) => {
  const {
    close, high, low, open,
  }: CandleValue = {
  ...
  return <>
    ...
    ></rect>
    {
      ...
      <line x1={centerY} x2={centerY} y1={Math.min(open,close)} y2={high} stroke={shadowColor} strokeWidth={shadowStrokeWidth} strokeLinecap="round"></line>
    }
    {
      value.low < Math.min(value.open,value.close) &&
      <line x1={centerY} x2={centerY} y1={Math.max(open,close)} y2={low} stroke={shadowColor} strokeWidth={shadowStrokeWidth} strokeLinecap="round"></line>
    }
  </>;
};

export const Candlestick: React.FC<CandlestickProps> = ({ theme: _theme, values, width,
  height, }) => {
  ...
  const {
    candleDecreasing, candlePadding, candleRaising, mode,
  } = theme;
  ...
  const hegihtRange = height - 2 * heightPadding;
  const minHeight = heightPadding;
  ...
  const heightPositionFormatter: CandleProps["heightPositionFormatter"] = (value: number) => {
  ...
  return <>
    ...
    >
      {/* <rect width={width} height={heightPadding} />
      <rect width={width} height={heightPadding} y={height - heightPadding} /> */}
      <g transform={`translate(0,${minHeight})`}>
        {
          values.map((x, i) => <>
            <g
              transform={`translate(${candleWidthStep * i},0)`}
            >
              <Candle theme={theme} value={x} {...{ heightPositionFormatter }} width={candleWidth} />
            </g>
          </>)
        }
      </g>
